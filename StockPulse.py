# StockPulse Web App by KAUSTUBH PANDEY
...
st.write('''# StockPulse ''')  # title
st.sidebar.image("Images/StockPulseLogo1.png", width=250,
                 use_column_width=False)  # logo
st.sidebar.write('''# StockPulse ''')

with st.sidebar: 
        selected = option_menu("Utilities", ["Stocks Performance Comparison", "Stock Price", "Stock Prediction", 'About'])
...
# read csv file
stock_df = pd.read_csv("StockPulseTickersData.csv")
...
if(selected == 'Stocks Performance Comparison'):  # if user selects 'Stocks Performance Comparison'
    ...
    dict_csv = pd.read_csv('StockPulseTickersData.csv', header=None, index_col=0).to_dict()[1]  # read csv file
...
# Stock Price Section Starts Here
elif(selected == 'Stock Price'):  # if user selects 'Stock Price'
...
# Stock Price Prediction Section Ends Here
    ...
    st.markdown('<p class="big-font">StockPulse is a web application that allows users to visualize Stock Performance Comparison, Stock Prices and Stock Price Prediction. This application is developed using Streamlit. Streamlit is an open source app framework in Python language. It helps users to create web apps for Data Science and Machine Learning in a short time. This Project is developed by Kaustubh Pandey. You can find more about the developer on their LinkedIn Profile shared below.<br> Cheers!</p>', unsafe_allow_html=True)
    st.subheader('Kaustubh Pandey [![Repo](https://badgen.net/badge/icon/LinkedIn?icon=linkedin&label)](https://www.linkedin.com/in/kaustubhpandey29/) ')
